...
const checkSession = async () => {
  ...
  console.log(details);
  isLoggedIn.value = details.user_id !== "" ? true : false;
  username.value = details.name;
  email.value = details.email;

  profile.set(details.user_id, details.name, details.email, details.isAdmin, details.isDeleted, details.isPremium);
...
<template>
  ...
  >
    <div class="max-w-6xl mx-auto flex flex-col items-center gap-3">
      ...
      <h1 class="dark:text-gray-100 text-gray-900 text-7xl font-bold tracking-wider">
        CodeFlick
      </h1>
      <div v-if="!isLoggedIn" class="dark:text-white/80 text-sm md:text-md italic">
